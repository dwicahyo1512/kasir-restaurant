<x-splade-component is="button-with-dropdown" dusk="bulk-actions-exports-dropdown" v-bind:close-on-click="true">
    ...
    <div class="min-w-max rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5">
        <div class="flex flex-col">
            <h3 v-if="table.hasSelectedItems" class="text-xs uppercase tracking-wide bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b">
            ...
            @foreach($table->getBulkActions() as $bulkAction)
                <button
                    ...
                    class="text-left w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 font-normal"
            ...
            @if($table->hasExports())
                <h3 class="text-xs uppercase tracking-wide bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b">
                    {{ __('export_results') }}
                </h3>
            @endif
